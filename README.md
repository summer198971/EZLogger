...
- 🚀 **零GC分配**: 发行模式下确保零垃圾回收分配
- 📊 **多级别控制**: 支持5个日志级别，与Unity LogType完全对齐
- 🔄 **异步处理**: 文件输出器内置异步处理，避免主线程阻塞 (也支持webgl单线程平台)
- 🌐 **服务器集成**: 支持日志发送到远程服务器 (需要部署日志服务器 待开发)
- 🎯 **Unity优化**: 专为Unity环境优化，支持帧数、堆栈跟踪等
- 🛡️ **系统监控**: 自动捕获Unity系统错误和异常，支持防重复机制
- 📡 **错误上报**: 自动上报系统错误到服务器，便于线上问题追踪 (需要部署日志服务器 待开发)
...
| 平台 | 状态 | 特性支持 | 备注 |
|------|------|----------|------|
| 🖥️ **PC (Windows/Mac/Linux)** | ✅ 完全支持 | 全功能 | 包括异步文件输出、服务器上报、系统监控 |
| 📱 **Android** | ✅ 完全支持 | 全功能 | 优化移动设备性能，支持持久化日志 |
| 🍎 **iOS** | ✅ 完全支持 | 全功能 | 支持iOS沙盒环境下的日志存储 |
| 🌐 **WebGL** | ✅ 特殊支持 | 单线程优化 | 自动切换同步模式，支持浏览器存储 |

> 💡 **WebGL特殊优化**: 针对WebGL单线程环境，EZ Logger自动禁用异步处理，确保在浏览器中稳定运行。

### 🎮 WebGL演示项目

体验EZ Logger在WebGL平台的完整功能：

- **📱 在线演示**: [https://summer198971.github.io/ezlogger-webgl-demo/](https://summer198971.github.io/ezlogger-webgl-demo/)
- **📂 源码仓库**: [EZLogger WebGL Demo](https://github.com/summer198971/ezlogger-webgl-demo)
...
详细配置说明请参考：[项目设置指南](Documentation~/project-settings-guide.md)
