...
EZ Logger 提供了完整的 Unity Project Settings 集成，让你可以通过图形界面配置所有功能。

### 打开设置界面

```
Unity菜单 → Edit → Project Settings → EZ Logger
或
菜单 Tools → EZ Logger → Open Settings
```
...
- **基础配置**: 日志级别、性能模式、堆栈跟踪
- **异步处理**: 异步写入、队列大小、缓冲区
- **Unity控制台**: 颜色、帧数显示、线程ID
- **文件输出**: 目录、文件大小、轮转、压缩
- **服务器上报**: URL、超时、批量发送、压缩
- **系统监控**: 自动错误捕获、防重复处理
- **扩展设置**: 设备信息、性能信息收集
